import './IngredientsComponent.css'

function IngredientsComponent() {
  return (
    <div className="ingredients-wrapper">
        <div className="ingredients-header">
            <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#006045">
                <path d="M281.5-201.5Q200-283 200-400q0-94 55.5-168.5T401-669q-20-5-39-14.5T328-708q-33-33-42.5-78.5T281-879q47-5 92.5 4.5T452-832q23 23 33.5 52t13.5 61q13-31 31.5-58.5T572-828q11-11 28-11t28 11q11 11 11 28t-11 28q-22 22-39 48.5T564-667q88 28 142 101.5T760-400q0 117-81.5 198.5T480-120q-117 0-198.5-81.5Zm340-57Q680-317 680-400t-58.5-141.5Q563-600 480-600t-141.5 58.5Q280-483 280-400t58.5 141.5Q397-200 480-200t141.5-58.5ZM480-400Z"/>
            </svg>
            <h3>Ingredients</h3>
        </div>
        <div className="ingredients">
            <div className="ingredient">
                <span className="ingredient-name">Pineaple</span>
                <span className="amount">1/2 cup</span>
            </div>
            <div className="ingredient">
                <span className="ingredient-name">Mango</span>
                <span className="amount">1/2 cup</span>
            </div>
            <div className="ingredient">
                <span className="ingredient-name">Banana</span>
                <span className="amount">1 small</span>
            </div>
            <div className="ingredient">
                <span className="ingredient-name">Orange juice</span>
                <span className="amount">3/4 cup</span>
            </div>
            <div className="ingredient">
                <span className="ingredient-name">Greek yogurt</span>
                <span className="amount">1/4 cup</span>
            </div>
            <div className="ingredient">
                <span className="ingredient-name">Chia seeds</span>
                <span className="amount">1 tsp</span>
            </div>
            <div className="ingredient">
                <span className="ingredient-name">Ice</span>
                <span className="amount">1/2 cup</span>
            </div>
        </div>
    </div>
  )
}

export default IngredientsComponent
